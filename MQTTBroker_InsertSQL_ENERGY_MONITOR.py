...
db_config = {
    'dbname': 'c102',
    'user': 'csti',
    'password': '*1frj7csti7gov7br',
    'host': 'localhost',
    'port': '5432'
}

# ==== Fun    o para inserir no PostgreSQL ====
def insert_kwh_consumption(cursor, location, accumulated, value):
    cursor.execute("""
        INSERT INTO KWH_CONSUMPTION (location, accumulated, value)
        VALUES (%s, %s, %s)
    """, (location, accumulated, value))
...
def on_message(client, userdata, msg):
    try:
        ...
        if msg.topic == "C102\\HARDWARE_SENSORS":
            ...
            insert_kwh_consumption(cursor, payload['location'], payload['accumulated'], payload['value'])
            print("Dados do sensor registrados")
        ...
        cursor.close()
        conn.close()
        print(f"Dados inseridos do t  pico {msg.topic}")
...
#client.username_pw_set("usuario", "senha")  # Se necess  rio
client.on_connect = on_connect
client.on_message = on_message

client.connect("10.11.102.123", 8883, 60)  # Porta MQTT TLS
#client.connect("10.11.1.110", 8883, 60)
